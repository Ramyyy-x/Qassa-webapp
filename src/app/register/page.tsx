...
export default async function RegisterPage({
  ...
  return (
    <div className="flex min-h-screen flex-col justify-center items-center px-6 py-12 bg-black text-white relative selection:bg-amber-500/30 overflow-hidden">
      ...
      <Link href="/" className="absolute top-4 right-4 sm:top-8 sm:right-8 flex items-center gap-2 text-zinc-400 hover:text-amber-500 transition-colors font-bold bg-white/5 px-3 py-2 sm:px-4 sm:py-2 text-sm sm:text-base rounded-xl border border-white/10 backdrop-blur-md z-10">
        <ArrowRight size={18} className="sm:w-5 sm:h-5" />
        <span className="hidden sm:inline">العودة للرئيسية</span>
        <span className="sm:hidden">عودة</span>
      </Link>
      
      <div className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700 mt-12">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl font-black text-white tracking-tight">إنشاء حساب جديد</h2>
          <p className="mt-3 text-center text-zinc-400 text-lg">انضم إلى عملاء صالون قَصَّة</p>
        </div>

        <form className="bg-zinc-900/60 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-5" action={signup}>
          ...
          <div>
            <label htmlFor="name" className="block text-sm font-bold text-zinc-300 mb-3 ml-1">
              الاسم الكامل
            </label>
            <input
              id="name"
              ...
              type="text"
              required
              className="w-full bg-black/30 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all"
        ...
        <p className="mt-8 text-center text-zinc-400 font-medium">
          لديك حساب مسبقاً؟{' '}
          <Link href="/login" className="font-black text-amber-400 hover:text-amber-300 transition-colors drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]">
            تسجيل الدخول
          </Link>
        </p>
      </div>
