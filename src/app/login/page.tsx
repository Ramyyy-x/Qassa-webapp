    ...
    <div className="flex min-h-screen flex-col justify-center items-center px-6 py-12 bg-black text-white relative selection:bg-amber-500/30 overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="flex flex-col items-center mb-10">
          ...
          <h2 className="text-3xl font-black text-white tracking-tight">قَصَّة</h2>
          <p className="mt-3 text-center text-zinc-400 text-lg">مرحباً بك مجدداً</p>
        </div>

        <form className="bg-zinc-900/60 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-6" action={login}>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <label htmlFor="password" className="block text-sm font-bold text-zinc-300 ml-1">
                كلمة المرور
              </label>
              <Link href="/forgot-password" className="text-sm font-bold text-amber-500 hover:text-amber-400 transition-colors">
                نسيت كلمة المرور؟
              </Link>
            </div>
            <input
        ...
        <p className="mt-8 text-center text-zinc-400 font-medium">
          ليس لديك حساب؟{' '}
          <Link href="/register" className="font-black text-amber-400 hover:text-amber-300 transition-colors drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]">
            إنشاء حساب جديد
          </Link>
        </p>
      </div>
